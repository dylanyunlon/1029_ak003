...
//#define BT_AUTH_STATUS   "sys.bt.auth"
#define DATA_BUF_LEN     2048

#if BT_MODULE == BT_MODULE_GOCSDK
#define BLINK_DIR        "/data/goc/"		// 顾凯授权文件
#define BLINK_LIC        BLINK_DIR ".goc_database"
#else
#define BLINK_DIR        "/data/blink/"		// 诚谦授权文件
...
#define CALL_SOCKET                "/dev/socket/bt_mic_socket" // mic数据
...
#define BT_CMD_QUERY_HFP_STATE 		"CY"		// 查询HFP状态
...
#define BT_CMD_DEL_MATCH			"CV"		// 删除配对列表
...
#if BT_MODULE == BT_MODULE_GOCSDK
#define BT_CMD_RECORD_RECENT 		"PJ"		// 同步最近通话记录
#else
...
#if BT_MODULE == BT_MODULE_GOCSDK
#define BT_CMD_MUSIC_PLAY 			"MA"		// 播放
#define BT_CMD_MUSIC_STOP 			"MB"		// 暂停
#else
...
typedef struct {
	...
	bt_call_state_e call_state = E_BT_CALL_STATE_IDLE;
	std::string call_num;

	bt_music_t music_info;

	std::map<std::string, bt_msg> scan_dev_map;    // map[addr] = name
	std::map<std::string, bt_msg> matched_dev_map;
...
static fy::localsocket _s_call_sock;
...
//联系人首字符排序
static void initial_sort() {
	bt_contact_t _contact;
	int size = m_contact_list.size() - 1;
	for (int i = 0; i < size; i++) {
		for (size_t j = i + 1; j < m_contact_list.size(); j++) {
			if (tolower(m_contact_list[i].pinyin[0]) > tolower(m_contact_list[j].pinyin[0])) {
				_contact = m_contact_list[i];
				m_contact_list[i] = m_contact_list[j];
				m_contact_list[j] = _contact;
			}
		}
	}
}
...
static void _audio_record_cb(const void *data, uint32_t size) {
	if (_s_bt_info.call_state == E_BT_CALL_STATE_TALKING) {
		int ret = _s_call_sock.write(data, size);
		if (ret <= 0) {
			LOGE("[bt] call sock write err ret %d\n", ret);
		}
	}
}
...
static void _proc_power_on(const char *data, int len) {
	...
	switch (data[0]) {
	case '0':		// close
		_proc_power_off("IS0", strlen("IS0"));
		break;
	case '1':		// open
		_send_cmd(BT_CMD_QUERY_MODIFY_NAME);
		goto BT_EN;
	case '2':		// opening
	case '3':		// closeing
...
static void _proc_power_off(const char *data, int len) {
	...
	//audio::set_system_vol(audio::get_system_vol());
	if (_s_bt_info.calling) { audio::handle_phone(E_AUDIO_TYPE_BT_PHONE, false); }// 通话状态强制结束——>音量恢复
	{
...
//	sys::setting::set_bt_name(_s_bt_info.dev_info.name);
...
static void _proc_scan(const char *data, int len) {
	...
	std::string type = std::string(data, 1);
...
//	LOGD("[bt] scan addr: %s, name: %s\n", addr.c_str(), name.c_str());
	...
	bt_msg s_msg;
	s_msg.name = name;
	s_msg.status = "Available device";
	if (addr == get_connect_dev().addr) {
		return;
	}

	LOCK_OP(_s_info_mutex, _s_bt_info.scan_dev_map[addr] = s_msg);
...
static void _proc_match_device(const char *data, int len) {
	...
	bt_msg s_msg;
	s_msg.name = name;
	s_msg.status = addr == get_connect_dev().addr ? "Connected" : "Paired";
	s_msg.match_index = index - 48;   //char to int

	LOCK_OP(_s_info_mutex, _s_bt_info.matched_dev_map[addr] = s_msg);
...
static void _proc_hfp_state(const char *data, int len) {
	bool calling = false;
	LOGD("--%d-- --time:%s-- --%s-- \n", __LINE__, __TIME__, std::string(data, len).c_str());
...
//	LOCK_OP(_s_info_mutex, _s_bt_info.call_start_time = LLONG_MAX);
	...
	if (_s_bt_info.calling) { audio::handle_phone(E_AUDIO_TYPE_BT_PHONE, false); }
	_s_bt_info.calling = false;
//	audio::set_system_vol(audio::get_system_vol());			// 通话状态强制结束——>音量恢复
}
...
static void _proc_music_play_progress(const char *data, int len) {
	...
	// 8位16进制字符串， 前4位置，后4进度
	info.curpos = hexStr_to_int(std::string(data, 4));
	info.duration = hexStr_to_int(std::string(data+4, 4));

#if 0
	LOGD("--%d-- --%s-- std::string(data, 4) = %s, std::string(data+4, 4) = %s \n", __LINE__, __FILE__
			, std::string(data, 4).c_str(), std::string(data+4, 4).c_str());

	LOGD("--%d-- --%s-- curpos = %d, length = %d \n", __LINE__, __FILE__, info.curpos, info.duration);
#endif
...
static void _proc_speak_param(const char *data, int len) {
	// RT44100-2-16    蓝牙音乐播放参数
	// RT8000-1-16       蓝牙电话播放参数
}

static void _proc_car_sound(const char *data, int len) {
	LOGD("[bt] car sound\n");
	_s_bt_info.car_sound = true;
	call_vol(audio::get_call_vol());
...
//		SystemProperties::setInt(BT_AUTH_STATUS, 1);
...
static void _proc_open_recording(const char *data, int len) {
	if (!_s_call_sock.connect(CALL_SOCKET)) {
		LOGE("[bt] connect %s fail\n", CALL_SOCKET);
		return;
	}
	LOGD("[bt] connect %s ok\n", CALL_SOCKET);
	audio::record_start(16000, _audio_record_cb);
}

static void _proc_close_recording(const char *data, int len) {
	_s_call_sock.close();
	audio::record_stop();
	LOGD("[bt] disconnect %s\n", CALL_SOCKET);
}
...
static proc_fun _s_proc_fun_tab[] = {
	...
	"MC", _proc_open_recording, 	  // 开启录音
	"MD", _proc_close_recording,      // 停止录音
};
...
void del_scan_dev(bt_dev_t dev) {
	Mutex::Autolock _l(_s_info_mutex);
	_s_bt_info.scan_dev_map.erase(dev.addr);
}

bool get_scan_dev_by_index(int index, bt_dev_t &dev) {
	Mutex::Autolock _l(_s_info_mutex);

	std::map<std::string, bt_msg>::const_iterator it = _s_bt_info.scan_dev_map.begin();
	for (int i = 0; it != _s_bt_info.scan_dev_map.end(); ++i, ++it) {
		if (i == index) {
			dev.addr = it->first;
			dev.name = it->second.name;
			dev.status = it->second.status;
			return true;
		}
	}
	return false;
...
bool get_matched_dev_by_index(int index, bt_dev_t &dev) {
	...
	std::map<std::string, bt_msg>::const_iterator it = _s_bt_info.matched_dev_map.begin();
	for (int i = 0; it != _s_bt_info.matched_dev_map.end(); ++i, ++it) {
		if (i == index) {
			dev.addr = it->first;
			dev.name = it->second.name;
			dev.status = it->second.status;
			dev.match_index = it->second.match_index;
			return true;
		}
	}
	return false;
...
bool del_match_device(int index) {									/*const char *addr*/
	char cmd[64];
	snprintf(cmd, sizeof(cmd), "%s%d", BT_CMD_DEL_MATCH, index);
	return _send_cmd(cmd);
}

bool del_match_alldevice(){
	LOCK_OP(_s_info_mutex, _s_bt_info.matched_dev_map.clear());
	return _send_cmd(BT_CMD_DEL_MATCH);
}
...
bool set_mic_mute(bool mute) {
	return _send_cmd(BT_CMD_MIC_SWITCH);
}
...
bool query_state() {
	return _send_cmd(BT_CMD_QUERY_HFP_STATE);
}
...
bool query_name() {
	return _send_cmd(BT_CMD_QUERY_MODIFY_NAME);
}
...
bool music_pause() {
#if BT_MODULE == BT_MODULE_GOCSDK
	return _send_cmd(BT_CMD_MUSIC_STOP);
#else
	return _send_cmd(BT_CMD_MUSIC_PLAY);
#endif
}
...
//	LOCK_OP(_s_info_mutex, _s_bt_info.music_is_mute = mute);
//	if (!mute && !calling) {
//		audio::change_audio_type(E_AUDIO_TYPE_BT_MUSIC);
//	}
...
static bool _start_bt_service() {
	...
	// 等待虚拟串口创建完成
	WAIT(FILE_EXIST(BT_UART_DEV), 500, 5);
#endif
	return FILE_EXIST(BT_SERIAL);
}

static void _close_uart() {
	RETURN_IF_FAIL(_s_bt_fd >= 0)
	close(_s_bt_fd);
	_s_bt_fd = -1;
}
