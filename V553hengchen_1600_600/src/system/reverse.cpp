...
#define REVERSE_STATUS_NODE   "/sys/class/extcon/extcon0/state"
...
static std::set<reverse_camera_state_cb> _s_camera_state_cb_set;
static ReverseCameraState camera_state = REVERSE_CAMERA_STATE_DETECTED;
static Mutex _s_camera_state_mutex;

static void notify_camera_state(ReverseCameraState state);
  ...
  virtual bool threadLoop() {
    ...
    while (true) {
      ...
      if (ufd[0].revents & POLLIN) {
        ...
        int n;
        while ((n = _uevent_kernel_multicast_recv(event_fd, msg, UEVENT_MSG_LEN))
            > 0) {
          ...
          while (*pMsg) {
            ...
            if ((pTag = strstr(pMsg, "ZKVIDEO_BACK=")) != NULL) {
              _notify_status_cb(
                  (pTag[13] == '1') ?
                      REVERSE_STATUS_ENTER : REVERSE_STATUS_EXIT);
            }
#if 1
            if ((pTag = strstr(pMsg, "ZKVIDEO_DET=")) != NULL) {
//						  LOGE("%s", pMsg);
              procCameraState(
                  (pTag[12] == '1') ?
                      REVERSE_CAMERA_STATE_DETECTED :
                      REVERSE_CAMERA_STATE_UNDETECTED);
            }
#endif
  ...
  void procCameraState(ReverseCameraState now_state) {
    // 简单处理抖动，连续3次相同值才给出回调
    static ReverseCameraState last_state = now_state;
    ...
    const int JITTER_MAX = 3;
    if (score < JITTER_MAX) {
      ++score;
    }
    if (score == JITTER_MAX) {
      if (camera_state != now_state) {
        camera_state = now_state;
        notify_camera_state(now_state);
      }
      ++score;
    }
  }
...
static bool _does_reverse_status_enter() {
	...
	return strncmp(status, "ZKVIDEO_BACK=1", 14) == 0;
}
...
bool has_reverse_camera() {
  return camera_state == REVERSE_CAMERA_STATE_DETECTED;
}

void reverse_add_camera_state_cb(const reverse_camera_state_cb& cb) {
  Mutex::Autolock lock(_s_camera_state_mutex);
  _s_camera_state_cb_set.insert(cb);
}

void reverse_remove_camera_state_cb(reverse_camera_state_cb cb) {
  Mutex::Autolock lock(_s_camera_state_mutex);
  _s_camera_state_cb_set.erase(cb);
}

static void notify_camera_state(ReverseCameraState state) {
  Mutex::Autolock lock(_s_camera_state_mutex);
  for (auto cb : _s_camera_state_cb_set) {
    cb(state);
  }
}
