...
static LYLINK_TYPE_E _s_lyink_type;

static bool _s_lylink_is_authorized = false;
...
static void multiplySample(int16_t *samples, int numSameples, float factor) {
	int64_t newdata = 0;
	for (int i = 0; i < numSameples; i++) {
		newdata = samples[i] * factor;
		if (newdata <= -32768) {
			newdata = -32768;
		} else if (newdata >= 32767) {
			newdata = 32767;
		}
		samples[i] = newdata & 0xffff;
	}
}

#if MIX_MODE
static void _mix_mode_player_init(bool phoning, uint32_t channels, uint32_t rate) {
	if (phoning) {
...
static void _mix_mode_player_deinit(bool phoning) {
	if (phoning) {
...
static void _proc_audio(LYLINKAPI_EVENT evt, int para0, void *para1) {
...
#if MIX_MODE
		_mix_mode_player_init(para0 == AudioType_Phone, 2, 48000);
#else
...
#if !MIX_MODE
			audio_transfer_t &tts_transfer = _s_audio_info.audio_transfer[AudioType_TTS];
			audio_transfer_t &media_transfer = _s_audio_info.audio_transfer[AudioType_Media];
			if(media_transfer.player)
				zk_audio_multi_player_set_gain(media_transfer.player,audio::get_Navi_proportion());
			zk_audio_multi_player_set_gain(tts_transfer.player,audio::get_Navi_vol());
#endif
	...
	case LYLINK_AUDIO_STOP: {
		...
		// 处理auto导航播报音播放不完整问题
		if ((_s_lyink_type == LINK_TYPE_USBAUTO) || (_s_lyink_type == LINK_TYPE_WIFIAUTO)) {
...
#if MIX_MODE
		_mix_mode_player_deinit(para0 == AudioType_Phone);
#else
		...
		if (para0 == AudioType_TTS) {        // 导航播报
			...
			audio_transfer_t &media_transfer = _s_audio_info.audio_transfer[AudioType_Media];
			if (media_transfer.player)
				zk_audio_multi_player_set_gain(media_transfer.player,1);
#endif
...
#else
			multiplySample((int16_t *)para1, para0, 2.0);
...
static void _proc_video(LYLINKAPI_EVENT evt, int para0, void *para1) {
	...
	case LYLINK_VIDEO_HEAD:
		...
		if(_s_lyink_type == LINK_TYPE_MIRACAST) {// || _s_video_info.param.width == 0 || _s_video_info.param.height == 0) {
	...
	case LYLINK_VIDEO_DATA:{
		...
		if (_s_lyink_type == LINK_TYPE_MIRACAST) { // 兼容部分手机投屏无画面的问题
//			uint8_t *data = (uint8_t *) para1;
...
static void _lylink_callback(LYLINKAPI_EVENT evt, int para0, void *para1) {
	...
	case LYLINK_VIDEO_DATA:
		if (evt == LYLINK_VIDEO_START) {
			...
			if (_s_lyink_type == LINK_TYPE_MIRACAST) {
				return;
			}
		}
	...
	case LYLINK_LINK_ESTABLISH:
		...
		_s_lyink_type = (LYLINK_TYPE_E) para0;
		if (_s_lyink_type == LINK_TYPE_MIRACAST) {
			audio::change_audio_type(E_AUDIO_TYPE_LYLINK_MUSIC);
		} else if (_s_lyink_type == LINK_TYPE_WIFIAUTO) {
			bt::set_bt_mute(false);
		}
//		else if (_s_lyink_type == LINK_TYPE_WIFICP) {
//			bt::music_set_mute(true);
//		}
		break;

	case LYLINK_LINK_DISCONN:
		LOGD("--%d-- --%s-- LYLINK_LINK_DISCONN %s", __LINE__, __FILE__, lk::_link_type_to_str((LYLINK_TYPE_E) para0));
		if ((_s_lyink_type != LINK_TYPE_NULL) && (bt::get_bt_is_mute() != bt_music_is_mute)) {
			bt::set_bt_mute(bt_music_is_mute);
		}

		// 在通话时突然断开互联，清除通话状态
		if (_s_audio_info.call_state != CallState_Hang) {
			_s_audio_info.call_state = CallState_Hang;
			if (_s_audio_info.ringing) {
				_s_audio_info.ringing = false;
			}
		}
		_s_lyink_type = LINK_TYPE_NULL;
		_clear_video_info();
		break;
	...
	case LYLINK_UPDATE_TIME: {
		...
		LOGD("--刷新互联时间--zone %u-- --time:%lld---\n", param->zone, param->time);
		// 系统不记录时区
		struct tm *t;
		time_t timeStamp = param->time + param->zone * 60;
		t = localtime(&timeStamp);
		sys::setting::set_date_time(t);
	}
		break;
	default:
		break;
...
//		return false;
	...
	int width = LINK_VIEW_WIDTH;
	int height = LINK_VIEW_HEIGHT;
//	if (!sys::setting::is_link_screen_full()) {
//		width /= 2;
//	}
	int flags = LYLINK_FLAGS_VIDEO_LYLINK;
	if (sys::setting::get_link_mode() == E_LINK_MODE_ANDROIDAUTO) {
		flags |= LYLINK_FLAGS_MANUAL_BTCON;
	}

	if (sys::setting::get_sound_mode() == E_SOUND_MODE_LINK) {
		flags |= LYLINK_FLAGS_VIDEO_ONLY;
	}

	LYLINKAPI_PARAM params = { 0 };
	params.width = width;
	params.height = height;
	params.pos = sys::setting::is_driving_left() ? DrivingPosition_Left : DrivingPosition_Right;
	params.mode = MIX_MODE ? LYLINK_START_ASYNC_NODEC : LYLINK_START_ASYNC_NOMIX;
	params.callback = _lylink_callback;
	params.physical_width = width;
	params.physical_height = height;
...
bool start_lylink() {
	...
	fy::files::remove(MC_RESOLUTIONS_CONF);
	std::string data = MC_RESOLUTIONS_CONF_DATA;
	fy::files::save_data(MC_RESOLUTIONS_CONF, data.c_str(), data.size());

	// 设置miracast名称
	setenv("PRODUCT_NAME", sys::setting::get_miracast_name().c_str(), 1);

	// 设置airplay名称
	char script[128];
	sprintf(script, LINK_SCRIPT, sys::setting::get_airplay_name().c_str());
