...
static std::set<switch_mode_cb> _s_switch_mode_set;
static std::set<sound_mode_cb> _s_sound_mode_set;
static event_mode_e _s_event_mode;
static switch_mode_e _s_switch_mode = E_SWITCH_MODE_SETUP;
...
void set_switch_mode(switch_mode_e mode) {
	_s_switch_mode = mode;
	SET_FOREACH(_s_switch_mode_set, switch_mode_cb, it) {
		(*it)(mode);
	}
}

switch_mode_e get_switch_mode() {
	return _s_switch_mode;
}

void add_switch_mode_cb(switch_mode_cb cb) {
	_s_switch_mode_set.insert(cb);
}

void remove_switch_mode_cb(switch_mode_cb cb) {
	_s_switch_mode_set.erase(cb);
}
