...
static void onUI_init() {
	...
	if (val == 1) {  // 关机
		...
		mLogoTextViewPtr->setVisible(false);

		// 停掉各个服务
		STOP_SERVICE(BT_SERVICE);
		STOP_SERVICE(LINK_SERVICE);
		STOP_SERVICE(NET_P2P_SUPP);
		STOP_SERVICE(NET_AP_SUPP);
		...
		mActivityPtr->registerUserTimer(RESTART_TIMER, 1000);
	}
}
...
static bool onUI_Timer(int id){
	switch (id) {
	case RESTART_TIMER:
		SystemProperties::setInt(SHUTDOWN_PROP_KEY, 1);
		// 重启进程
		RESTART_SERVICE("zkswe");
		return false;
	default:
		break;
	}
