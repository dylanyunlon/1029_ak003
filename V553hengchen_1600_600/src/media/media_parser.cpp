...
static void _convert(char *dst, const uint8_t *src, int len) {
	if (_is_utf8_str(src, len)) {
		memcpy(dst, src, len);
		dst[len] = '\0';
	} else if (_is_gbk_str(src, len)) {
		strcpy(dst, gbk_to_utf8((const char *) src, len).c_str());
	}
}
...
//static bool format_filter(const char *file) {
//	const char* omitted_format[] = {".flac", ".wav"};		//忽略解析的格式
//	std::string filename = file;
//	std::string suffix = filename.substr(filename.rfind("."));
//	for (int i=0; i<(int)TAB_SIZE(omitted_format); i++) {
//		if (omitted_format[i] == suffix) {
//			return true;
//		}
//	}
//	return false;
...
//	}
	...
	_convert(info->title, mi.title, mi.titlesz);
	_convert(info->artist, mi.artist, mi.artistsz);
	_convert(info->album, mi.album, mi.albumsz);
...
//	}
	...
	if (mi.pAlbumArtBuf && (mi.nAlbumArtBufSize > 0)) {
		...
		if (pf) {
			if ((int)fwrite(mi.pAlbumArtBuf, 1, (int)mi.nAlbumArtBufSize, pf) == (int)mi.nAlbumArtBufSize) {
				ret = true;
			}
			fclose(pf);
		}
