...
static Mutex _s_media_file_mutex;
static bool _s_scan_started;

static storage_info_t _s_storage_info_tab[] = {
	{ E_STORAGE_TYPE_SD, "/mnt/extsd", false },
	{ E_STORAGE_TYPE_USB1, "/mnt/usb1", false },
	{ E_STORAGE_TYPE_USB2, "/mnt/usbotg", false },
};

static inline storage_type_e _get_storage_type(const char *root_path, bool isMount) {
	for (int i = 0; i < (int) TAB_SIZE(_s_storage_info_tab); ++i) {
		if (strcmp(_s_storage_info_tab[i].root_path, root_path) == 0) {
			_s_storage_info_tab[i].isMount = isMount;
...
static void _scan_cb(const char *file, void *user_data) {
	...
	const char *audio_suffix_tab[] = {
		"wav", "mp3", "ogg", "flac", "aac", "ape", "wma", "wv", NULL
	};
	if (_is_match_suffix(suffix, audio_suffix_tab)) {
		//LOGD("audio file: %s\n", file);
		Mutex::Autolock _l(_s_media_file_mutex);
...
static void _message_handler(const fy::message *msg, const void *user_data) {
	...
	case MSG_SCAN: {
		...
		const char *root_path = get_root_path(type);
		if (!root_path) {
			break;
		}
...
protected:
	virtual void notify(int what, int status, const char *msg) {
		...
		case MountMonitor::E_MOUNT_STATUS_MOUNTED: {
			...
			storage_type_e type = _get_storage_type(msg, true);
		...
		case MountMonitor::E_MOUNT_STATUS_UNMOUNTING: {
			...
			storage_type_e type = _get_storage_type(msg, false);
			if (type != E_STORAGE_TYPE_INVALID) {
				...
				if (root_path.compare(0, root_path.size(), play_file, 0, root_path.size()) == 0) {
					...
					LOGD("--%d-- --%s-- E_MOUNT_STATUS_UNMOUNTING+++\n", __LINE__, __FILE__);
					music_stop();
					LOGD("--%d-- --%s-- E_MOUNT_STATUS_UNMOUNTING---\n", __LINE__, __FILE__);
				}
...
void init() {
	setenv("ZKMEDIA_MPP_SYS_DISABLE", "1", 1);
	setenv("MPP_AO_DAUDIO0_DISABLE", "1", 1);
	setenv("ZKMEDIA_VIDEO_DISABLE_STOP_EOF", "1", 1);

	// 指定h264播放通道 chn1 lyr0
	setenv("ZKMEDIA_H264_LAYER", "4", 1);

	audio::init();
	music_init();
	zk_h264_player_preload();

	for (size_t i = 0; i < TAB_SIZE(_s_storage_info_tab); ++i) {
		_s_storage_info_tab[i].isMount = MOUNTMONITOR->isMounted(_s_storage_info_tab[i].root_path);
	}
...
bool get_storage_isMount(const char *root_path) {
	for (size_t i = 0; i < TAB_SIZE(_s_storage_info_tab); ++i) {
		if (strcmp(_s_storage_info_tab[i].root_path, root_path) == 0) {
			return _s_storage_info_tab[i].isMount;
		}
	}
	return false;
}
...
const char* get_root_path(storage_type_e type) {
	for (size_t i = 0; i < TAB_SIZE(_s_storage_info_tab); ++i) {
