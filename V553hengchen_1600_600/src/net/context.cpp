...
void init() {
	...
	mkdir(NET_HOSTAPD_DIR,0666);
	mkdir(NET_UDHCPD_DIR,0666);

	std::string device_name = sys::setting::get_dev_name();
	std::string carplay_ap_name = sys::setting::get_carplay_ap_name();
//	std::string airplay_ap_name = sys::setting::get_airplay_ap_name();
	std::string wifivideo_ap_name = sys::setting::get_wifivideo_ap_name();

	if (fy::files::size_of(NET_HOSTAPD_CONF_FILE) <= 0) {
		std::string data = fy::format(NET_HOSTAPD_CONF_DATA, carplay_ap_name.c_str());
		fy::files::save_data(NET_HOSTAPD_CONF_FILE, data.c_str(), data.size());
	}
	if (fy::files::size_of(NET_HOSTAPD_AP_CONF_FILE) <= 0) {
		std::string data = fy::format(NET_HOSTAPD_AP_CONF_DATA, carplay_ap_name.c_str());
		fy::files::save_data(NET_HOSTAPD_AP_CONF_FILE, data.c_str(), data.size());
	}
	if (fy::files::size_of(NET_HOSTAPD_WV_CONF_FILE) <= 0) {
        std::string data = fy::format(NET_HOSTAPD_CONF_DATA, wifivideo_ap_name.c_str());
        fy::files::save_data(NET_HOSTAPD_WV_CONF_FILE, data.c_str(), data.size());
	}
	...
	if (fy::files::size_of(NET_UDHCPD_CONF_FILE) <= 0) {
		fy::files::save_data(NET_UDHCPD_CONF_FILE, NET_UDHCPD_CONF_DATA, strlen(NET_UDHCPD_CONF_DATA));
	}
	if (fy::files::size_of(NET_UDHCPD_WV_CONF_FILE) <= 0) {
		fy::files::save_data(NET_UDHCPD_WV_CONF_FILE, NET_UDHCPD_WV_CONF_DATA, strlen(NET_UDHCPD_WV_CONF_DATA));
	}
	...
	if (!wifi_on) {
		if (sys::setting::get_link_mode() == E_LINK_MODE_MIRACAST || sys::setting::get_link_mode() == E_LINK_MODE_AICAST) {
