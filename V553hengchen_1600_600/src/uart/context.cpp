...
#define SET_VOLUME_DEBUG	1	// 划分mcu音量等级1-14/1-16
...
static uint8_t _arr_field[] = {0xa, 0xa, 0x14, 0x14, 0x00};// , 0x00
...
#if SET_VOLUME_DEBUG
	// 四舍五入计算整数等级 (0-10)
	uint8_t level = (uint8_t)(10.0 * vol + 0.5);
	// 音量值映射表 [0,1,2,3,4,5,6,8,10,12,14]
	static const uint8_t vol_map[11] = {0, 1, 2, 3, 4, 6, 8, 10, 12, 14, 16};
	uint8_t target_vol = vol_map[level];
    LOCK_OP(sys_voice._s_vol_mutex, sys_voice.volume = target_vol)
#else
	LOCK_OP(sys_voice._s_vol_mutex, sys_voice.volume = (uint8_t)(10.0 * vol + 0.1))		//((vol < 0.1) ? 0 : (20.0 * vol + 19))
#endif
	uint8_t data[] = { sys_voice.channel, sys_voice.volume };
	LOGD("volume = %d", sys_voice.volume);
...
bool set_touch_sound(bool status) {
	uint8_t data[1] = {0};
	data[0] = status;
	return _send_frame_data(CMD_KEY_TONE, data, TAB_SIZE(data));
}
